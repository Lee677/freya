...
(function (global) {
  ...
  /* ---------------------------------------------------------------------
   * PHOTO RIG — printable parts
   * ------------------------------------------------------------------- */

  /* Calibration frame: a flat rectangular frame whose INNER corners are at a
   * known spacing. Photograph the tool inside it, click the four inner
   * corners, and the app rectifies the perspective and sets the scale. */
  GF.buildTarget = function (o) {
    const innerW = o.innerW || 200, innerD = o.innerD || 150;
    const fw = o.frameW || 12, t = o.thickness || 3;
    let f = box(innerW + 2 * fw, innerD + 2 * fw, t, 0, 0, 0);
    f = keep(f.subtract(box(innerW, innerD, t + 2, 0, 0, -1)));
    // corner nibs pointing inward mark the exact reference points
    const nib = Math.min(6, fw * 0.6);
    const nibs = [];
    for (const sx of [-1, 1]) for (const sy of [-1, 1]) {
      nibs.push(box(nib, nib, t + 1.2, sx * (innerW / 2 + nib / 2 - nib), sy * (innerD / 2 + nib / 2 - nib), 0));
    }
    f = keep(f.add(unionAll(nibs)));
    return { solid: f, info: { innerW: innerW, innerD: innerD, outerW: innerW + 2 * fw, outerD: innerD + 2 * fw } };
  };

  /* Camera stand: base + stacking column segments + overhead phone tray.
   * Returns a list of printable parts. */
  GF.buildStand = function (o) {
    const height = o.height || 320;
    const col = o.column || 22;         // column cross-section
    const wallT = 3;
    const bedZ = Math.max(60, o.bedZ || 200);
    const phoneW = o.phoneW || 78;
    const phoneT = o.phoneT || 11;
    const reach = o.reach || 110;
    const lensX = o.lensX == null ? 22 : o.lensX;  // from the tray's left edge
    const lensY = o.lensY == null ? 20 : o.lensY;  // from the tray's back edge
    const lensD = o.lensD || 34;
    const parts = [];
    const peg = col - 2 * wallT - 0.35;   // plug between segments (0.35 fit gap)
    const pegH = 12;

    // --- base: most of the plate reaches out under the arm, or the whole
    // thing tips forward the moment a phone goes in the tray ---
    const baseW = Math.max(150, col * 6);
    const baseBack = 45, baseFront = Math.max(90, reach * 0.85);
    const baseD = baseBack + baseFront;
    let base = box(baseW, baseD, 6, 0, (baseFront - baseBack) / 2, 0);
    base = keep(base.add(box(col + 2 * wallT, col + 2 * wallT, 26, 0, 0, 0)));
    base = keep(base.subtract(box(col + 0.35, col + 0.35, 22, 0, 0, 6)));
    parts.push({ name: 'stand_base', solid: base, qty: 1, note: baseW + ' × ' + baseD.toFixed(0) + ' mm footprint' });

    // --- column segments ---
    const segs = Math.max(1, Math.ceil(height / (bedZ - 15)));
    const segH = height / segs;
    let colSeg = box(col, col, segH, 0, 0, 0);
    colSeg = keep(colSeg.add(box(peg, peg, pegH, 0, 0, segH)));
    colSeg = keep(colSeg.subtract(box(peg + 0.35, peg + 0.35, pegH + 0.4, 0, 0, -0.2)));
    parts.push({ name: 'stand_column_segment', solid: colSeg, qty: segs, note: segH.toFixed(1) + ' mm each' });

    // --- arm + phone tray ---
    const trayW = phoneW + 2 * wallT + 0.6;
    const trayD = Math.max(70, phoneW * 0.9);
    let arm = box(col, reach, 10, 0, reach / 2 - col / 2, 0);
    arm = keep(arm.add(box(col + 2 * wallT, col + 2 * wallT, 18, 0, 0, -8)));
    arm = keep(arm.subtract(box(col + 0.35, col + 0.35, 16, 0, 0, -9)));
    let tray = box(trayW, trayD, 4, 0, reach - trayD / 2, 6);
    tray = keep(tray.add(box(trayW, wallT, 4 + phoneT, 0, reach - trayD + wallT / 2, 6)));           // back stop
    tray = keep(tray.add(box(wallT, trayD, 4 + phoneT, -trayW / 2 + wallT / 2, reach - trayD / 2, 6))); // left rail
    tray = keep(tray.add(box(wallT, trayD, 4 + phoneT, trayW / 2 - wallT / 2, reach - trayD / 2, 6)));  // right rail
    const lens = box(lensD, lensD, 12, -trayW / 2 + lensX, reach - lensY, 2);
    tray = keep(tray.subtract(lens));
    arm = keep(arm.add(tray));
    parts.push({ name: 'stand_arm_tray', solid: arm, qty: 1 });

    // every part sits on the bed
    for (const p of parts) {
      const bb = p.solid.boundingBox();
      const z0 = (bb.min && bb.min[2] != null) ? bb.min[2] : (bb.min ? bb.min.z : 0);
      if (z0) p.solid = keep(p.solid.translate([0, 0, -z0]));
    }

    return {
      parts: parts,
      info: {
        height: height, segments: segs, segH: segH, reach: reach, trayW: trayW,
        baseW: baseW, baseD: baseD
      }
    };
  };
